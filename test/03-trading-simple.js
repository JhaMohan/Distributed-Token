...
contract('Simple Order Tests', function (accounts) {
    before(function() {
        ...
        return exchange.deployed().then(function (instance) {
            ...
            return instanceExchange.depositEther({from: accounts[0], value: web3.utils.toWei(new web3.utils.BN(3), "ether")});
    ...
    it("should be possible to add a limit buy order", function () {
        ...
        return exchange.deployed().then(function (instance) {
         ...
         //   console.log(orderBook.length);
            assert.equal(Object.keys(orderBook).length, 2, "BuyOrderBook should have 2 elements");
            assert.equal(orderBook[0].length, 0, "OrderBook should have 0 buy offers");
            return myExchangeInstance.buyToken("FIXED", web3.utils.toWei(new web3.utils.BN(1), "finney"), 5);
        }).then(function(txResult) {
            /**
             * Assert the logs
             */
    ...
    it("should be possible to add three limit buy orders", function () {
        ...
        return exchange.deployed().then(function (exchangeInstance) {
            ...
            assert.equal(txResult.logs[0].event, "LimitBuyOrderCreated", "The Log-Event should be LimitBuyOrderCreated");
            return myExchangeInstance.buyToken("FIXED", web3.utils.toWei(new web3.utils.BN(1.4), "finney"), 5); //we add a new offer in the middle
        }).then(function(txResult) {
            assert.equal(txResult.logs[0].event, "LimitBuyOrderCreated", "The Log-Event should be LimitBuyOrderCreated");
            return myExchangeInstance.getBuyOrderBook.call("FIXED");
        }).then(function(orderBook) {
    ...
    it("should be possible to add two limit sell orders", function () {
        ...
        return exchange.deployed().then(function (instance) {
            ...
            return myExchangeInstance.sellToken("FIXED", web3.utils.toWei(new web3.utils.BN(3), "finney"), 5);
        }).then(function(txResult) {
            /**
             * Assert the logs
             */
            ...
            return myExchangeInstance.sellToken("FIXED", web3.utils.toWei(new web3.utils.BN(6), "finney"), 5);
        }).then(function(txResult) {
            return myExchangeInstance.getSellOrderBook.call("FIXED");
        }).then(function(orderBook) {
    ...
    it("should be possible to create and cancel a buy order", function () {
        ...
        return exchange.deployed().then(function (instance) {
            ...
            orderBookLengthBeforeBuy = orderBook[0].length;
            return myExchangeInstance.buyToken("FIXED", web3.utils.toWei(new web3.utils.BN(2.2), "finney"), 5);
        }).then(function(txResult) {
            /**
             * Assert the logs
             */
