...
contract('Exchange Order Tests', function (accounts) {
    ...
    it("should be possible to add fully fulfill buy orders", function () {
        ...
        return exchange.deployed().then(function (instance) {
            ...
            assert.equal(Object.keys(orderBook).length, 2, "getSellOrderBook should have 2 elements");
            assert.equal(orderBook[0].length, 0, "OrderBook should have 0 buy offers");
            return myExchangeInstance.sellToken("FIXED", web3.utils.toWei(new web3.utils.BN(2), "finney"), 5, {from: accounts[1]});
        }).then(function(txResult) {
            /**
             * Assert the logs
             */
            ...
            return myExchangeInstance.buyToken("FIXED", web3.utils.toWei(new web3.utils.BN(3), "finney"), 5);
        }).then(function(txResult) {
