...
contract('Exchange Basic Tests', function (accounts) {
    ...
    it("should be possible to Deposit and Withdrawal Ether", async () =>{
        var myExchangeInstance;
 
        var balanceBeforeTransaction =await web3.eth.getBalance(accounts[0]);
        var balanceAfterDeposit;
        var balanceAfterWithdrawal;
        var gasUsed = 0;
        ...
        return exchange.deployed().then(function (instance) {
            myExchangeInstance = instance;
            return myExchangeInstance.depositEther({from: accounts[0], value: web3.utils.toWei(new web3.utils.BN(1), "ether")});
        }).then(async (txHash)=> {
           
            let result1 =  await web3.eth.getTransaction(txHash.receipt.transactionHash) 
            

            gasUsed += txHash.receipt.cumulativeGasUsed * result1.gasPrice; //here we have a problem
            ...
            assert.equal(Number(balanceInWei), 0, "There is no ether available anymore");
            assert.isAtLeast( Number(balanceAfterWithdrawal), Number(balanceBeforeTransaction )- gasUsed*2, "There is one ether available");
    ...
    it("should be possible to Withdraw Token", function () {
        ...
        return fixedSupplyToken.deployed().then(function (instance) {
            ...
            balancedTokenInExchangeBeforeWithdrawal = Number(balanceExchange);
